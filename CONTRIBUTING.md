...
| `signal:` | Добавление нового сигнала в `signals.json`/`SIGNALS.md` — не код, отдельная категория. Алгоритм добавления сигнала описан в [`CLAUDE.md`](CLAUDE.md#добавление-сигнала), не здесь. |
...
Коммиты с новым сигналом — отдельная категория, не код (N3 ARR v3: см.
`signal:` в таблице выше — раньше эта строка не была в таблице, добавлялась
только примером ниже, что создавало путаницу при быстром просмотре):
