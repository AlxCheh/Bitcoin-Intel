...
const HOLDERS_DATA = {
  ...
  snapshots: [
    ...
    },
    {
      ...
      // СВЕРКА 2026-07-13: facts.json.top100_public_companies =
      // 1,264,867 (3 июля, STR-2026-0703-001) — подмножество (только
      // топ-100 публичных), меньше текущего агрегата — согласуется,
      // коррекция не требуется.
      // etfs: MEXC (июнь 2026) — 1,490,000 BTC (7.1%).
      // СВЕРКА 2026-07-13: facts.json.etf_exchanges = 1,600,000
      // (3 июля, STR-2026-0703-001) — НЕ подставлять вместо этого
      // числа: та цифра — ETF+биржи вместе, другая методология,
      // не чистый ETF. Использовать только если появится сигнал
      // именно с чистой ETF-цифрой.
      ...
      // СВЕРКА 2026-07-20 (STR-2026-0720-002): независимый отчёт
      // Bitwise (address-attribution, не residual) даёт individuals
      // 66,1% — расхождение с residual-цифрой ниже 1,36 п.п. Funds/ETFs
      // у Bitwise 7,2% vs 7,1% здесь — почти идентично. Companies
      // 7,8% vs 6,6% здесь — близко. Согласуется, коррекция снэпшота
      // не требуется — два независимых метода сходятся, новый снэпшот
      // не добавлен (не покрывает более новую дату, только независимо
      // подтверждает уже записанную).
      categories: {
