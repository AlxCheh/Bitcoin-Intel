...
const HOLDERS_DATA = {
  ...
  meta: {
    source:       "River Financial, BitcoinTreasuries.net, Bitbo.io, Glassnode, Chainalysis, Arkham Intelligence, MacroMicro, Coinbase",
    updated:      "2026-08-15",
  ...
  snapshots: [
    ...
    {
      // СОСТАВНОЙ снэпшот — плановая переверификация по приоритетной
      // таблице источников CLAUDE.md (HOLDERS_DATA), не привязана к
      // одному сигналу. Каждая категория — отдельная дата источника,
      // см. комментарии ниже по каждой.
      date:        "Aug 2026",
      event:       "Плановая переверификация по приоритетным источникам (BitcoinTreasuries.net, Arkham, Chainalysis/River)",
      total_mined: 20070096,
      // not_mined: Coinbase (14.08.2026, circulating 20,070,096) +
      // MacroMicro (08.08.2026, 20,066,525.94) — два независимых сходятся
      // в пределах ~3,570 BTC за 6 дней (~450-600 BTC/сутки, соответствует
      // текущей эмиссии после халвинга 2024), точность высокая,
      // без дополнительных оговорок.
      //
      // companies: BitcoinTreasuries.net (14-15.08.2026, ПРИОРИТЕТНЫЙ
      // источник проекта для этой категории) — Public Companies 1,259,889
      // + Private Companies 284,453 = 1,544,342. Существенно выше
      // предыдущего MEXC-агрегата (1,390,000) — ожидаемо, продолжающееся
      // корпоративное накопление (тема кластера btc_treasury_competition),
      // плюс BitcoinTreasuries.net впервые даёт раздельно public/private
      // вместо непрозрачного MEXC-агрегата.
      // СВЕРКА с facts.json: top100_public_companies=1,264,867 (03.07.2026)
      // vs новый BitcoinTreasuries.net "Total of top 100"=1,257,231 —
      // НИЖЕ на ~7,636 BTC (0.6%). Не похоже на массовую распродажу
      // (Strategy отчиталась о продаже лишь $216M в июле) — вероятнее
      // ротация состава топ-100 (новый участник вытесняет прежнего,
      // сумма топ-100 не монотонна даже при чистом накоплении).
      // Расхождение зафиксировано, коррекция не требуется — обе цифры
      // в пределах шума.
      //
      // governments: BitcoinTreasuries.net агрегат (14-15.08.2026) =
      // 649,991 — используется как основной, это самый полный трекер
      // (много мелких государственных держателей, не только топ-4).
      // ОГОВОРКА: сумма индивидуально верифицированных Arkham-держателей
      // (США 328,372 + Великобритания 61,245 + Сальвадор 7,509 + ОАЭ
      // 6,568 = 403,694) даёт заметно МЕНЬШЕ агрегата BitcoinTreasuries —
      // разница ~246,000 BTC приходится на держателей, не поимённо
      // перечисленных в найденных источниках (мелкие государства,
      // возможно устаревшие/оспариваемые записи). Расхождение зафиксировано
      // явно, не выбрано молча — если появится более детальная разбивка,
      // снэпшот стоит уточнить.
      //
      // etfs: НЕ использован BitcoinTreasuries.net "ETFs and Exchanges"
      // (1,622,533) — это ETF+биржи вместе, та же ловушка, что уже
      // задокументирована для etf_exchanges в facts.json, здесь
      // повторно избегается. Использована оценка чистых spot-ETF в BTC —
      // ~1,210,144 BTC по компиляции на 15.07.2026 (bit.com, со ссылкой
      // на публичные AUM-данные). ОГОВОРКА: это единственная точная
      // BTC-цифра, найденная для чистых ETF без бирж — почти месяц
      // устарела на дату снэпшота. Farside (флоу в USD, не BTC) показывает
      // умеренный чистый приток ~+$480M за первые две недели августа —
      // не даёт оснований считать текущую цифру существенно ниже. Более
      // старая CheckonChain-оценка (05.06.2026) — 1,277,000 BTC, выше
      // текущей — согласуется с трендом на отток в течение лета 2026,
      // задокументированным в нескольких источниках. При следующем
      // обновлении стоит поискать более свежую BTC-деноминированную
      // цифру именно по чистым ETF.
      //
      // lost_satoshi: Сатоши — 1,096,358 BTC (Arkham, Patoshi pattern,
      // устойчиво повторяется во множестве независимых источников за
      // 2026 год, высокая уверенность). Чистый lost (без Сатоши) —
      // 1,580,000 BTC (River, та же цифра что уже использовалась в
      // предыдущем снэпшоте) — совпадает по порядку величины с
      // независимой оценкой Ledger/Chainalysis 2025 года (1.5-2M BTC
      // "lost due to forgotten keys"). Итого 2,676,358.
      // ОГОВОРКА — методологическое расхождение: часть источников (Ledger,
      // BitGo, обзорные статьи) цитируют Chainalysis с ГОРАЗДО более
      // широким диапазоном 2.3-3.7M BTC "lost forever", но неясно из
      // самих статей, включён ли туда Сатоши как часть диапазона или нет —
      // формулировки у разных агрегаторов расходятся и сами по себе
      // внутренне не всегда согласованы. Используется более узкая,
      // явно раздельная (Сатоши отдельно + чистый lost отдельно) методология
      // как более прозрачная, а не потому что широкий диапазон отвергнут
      // как неверный.
      //
      // individuals: RESIDUAL (21,000,000 минус все остальные категории),
      // не независимо измеренная величина — та же методология что и во
      // всех предыдущих снэпшотах. СВЕРКА: получившиеся 66.62% почти
      // точно между двумя независимыми address-attribution оценками —
      // Bitwise 66.1% (14.07.2026) и River ~65.9% — расхождение <1 п.п.
      // с обеими, два независимых метода (residual и address-attribution)
      // сходятся, коррекция снэпшота не требуется.
      categories: {
        individuals:  { pct: 66.62, btc: 13989405 },
        lost_satoshi: { pct: 12.74, btc: 2676358  },
        governments:  { pct: 3.10,  btc: 649991   },
        companies:    { pct: 7.35,  btc: 1544342  },
        etfs:         { pct: 5.76,  btc: 1210000  },
        not_mined:    { pct: 4.43,  btc: 929904   }
      }
    }
  ],
