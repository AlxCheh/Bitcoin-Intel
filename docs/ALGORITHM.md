...
## Версия: 2.0 · Обновлено: 2026-07-04
...
**`_get_contradicts()` читалa только `links.contradicts`** при `LEGACY_LINKS_ENABLED = True`. **Исправлено 2026-07-04, ALGORITHM_VERSION 2.1.1** (PATCH — bugfix без изменения замысла логики): после миграции на `relationships.json` (Фаза 0 завершена 2026-07-01) функция при `LEGACY_LINKS_ENABLED = False` безусловно возвращала `[]` — предупреждение об этом стояло здесь же двумя днями раньше миграции, но рефакторинг не был выполнен синхронно с флагом. Contradiction bonus был равен 0 для всех сигналов с 1 по 4 июля; anchor/tension selection по `MAX(contradicts)` не работал ни для одного кластера. Обнаружено вручную при инженерной сверке IRP Wave 2-5, не автоматикой — тестового покрытия на эту комбинацию не было (добавлено: `tests/unit/test_synthesizer.py`, 5 тестов). Сейчас `_get_contradicts(signal, contradicts_map)` при `LEGACY_LINKS_ENABLED = False` читает `contradicts_map`, построенный `_load_contradicts_map()` в `main()` и переданный в `synthesize_cluster()` как параметр (тот же паттерн §17, что `previous_synthesis`). Реальный эффект на 52 сигналах: confidence вырос для всех 5 кластеров (0.46-0.54 → 0.46-1.0), anchor сменился для 3 из 5.
