...
## Раздел 30. Anti-Corruption Layers

> IRP v1 Wave 3 / D02 (2026-07-01). Scope сознательно ограничен: краткое
> описание существующих границ между Python-слоями по факту кода, а не
> полноценный DDD Context Map (несколько bounded contexts с явными
> upstream/downstream отношениями и картой трансляций между ними) — тот
> откладывается в Technical Debt After MVP до масштабирования к 5+
> bounded contexts (см. `docs/IRP_v1.md` Residual Risk RR-04). На
> 2026-07-01 в проекте по факту один bounded context (нарративный движок
> Bitcoin Intel), поэтому формальный Context Map не добавляет ценности,
> которую не даёт уже раздел 22.

### 30.1 Что такое ACL в этом проекте

Anti-Corruption Layer — точка, где данные пересекают границу с моделью,
которую этот слой не контролирует (внешний JSON, другой язык, другой
формат), и должны быть переведены во внутреннюю модель слоя, не наоборот.
В этом проекте таких границ три, все уже описаны неформально в §22
(Dependency Rules) — этот раздел называет их явно паттерном ACL и
указывает, в каком файле живёт трансляция.

### 30.2 ACL-1: Внешний JSON → доменная модель Signal

**Граница:** аналитик (или, в перспективе, HTTP-клиент Backend Фазы 4)
пишет сигнал как произвольный JSON. `domain/` не должен доверять его
форме.

**Где трансляция:** `domain/validator.py::validate_signal()`, вызывается
из `scripts/add_signal.py` до записи в `signals.json`. FAIL LOUD —
исключение из иерархии `BitcoinIntelError`, а не тихая коррекция или
дефолт. После прохождения `validate_signal()` остальной код (`synthesizer.py`,
`contradiction_detector.py`) может полагаться на инварианты (обязательные
поля присутствуют, enum-значения из допустимого множества, `id` нужного
формата) без повторной проверки.

**Что это НЕ:** `validator.py` не трансформирует данные (не переименовывает
поля, не конвертирует типы) — только проверяет и отклоняет. В терминах
классического ACL это скорее пограничный guard/validator, чем
translator — трансляции формата в этом проекте нет, потому что внешний
формат (JSON-объект сигнала) и внутренний совпадают по структуре
(dataclass не используется для Signal — он остаётся dict). Граница всё
равно реальна: код по одну сторону не может полагаться на инварианты,
код по другую — может.

### 30.3 ACL-2: signals.json → SynthesisResult (Processing / Narrative)

**Граница:** `synthesizer.py` не должен знать о способе хранения
(`signals.json`, будущая БД) — принимает данные параметром, не читает
файл изнутри `synthesize_cluster()` (§22.3, ГРАНИЦА 2).

**Где трансляция:** `main()` в `synthesizer.py` — единственное место,
которое читает `signals.json` и передаёт результат внутрь. `SynthesisResult`
(dataclass) — внутренняя модель Narrative-слоя, не переиспользует форму
сигнала.

**Направление upstream/downstream:** `signals.json` — upstream (источник
истины), `SynthesisResult` — downstream-модель, которая не течёт назад.
Это соответствует ЗАПРЕЩЕНО в §22.2: `signals.json → synthesis_store/`
не существует как обратная зависимость.

### 30.4 ACL-3: synthesis_cache.json → UI (Narrative / Delivery)

**Граница:** `index.html`/JS не должен знать о `synthesizer.py`,
`synthesis_store/`, `relationships.json` — только о форме
`synthesis_cache.json` (§22.3, ГРАНИЦА 3). Это единственная граница
проекта, которая одновременно является границей между языками
(Python → JSON → JavaScript), а не только между модулями одного языка.

**Где трансляция:** запись — `synthesizer.py::_save_synthesis()`/`main()`
(Python-модель → JSON). Чтение — `loadSignals()`/рендер-функции в
`index.html` (JSON → DOM). Ни один JS-код не читает `synthesis_store/*.json`
напрямую — только агрегированный `synthesis_cache.json`.

**Известный источник дрейфа через эту границу:** ADR-010 документирует,
что часть scoring-логики продублирована в JS (`test_js_python_equivalence.py`
держит эквивалентность видимой, не гарантирует её автоматически для новых
правил) — сама ACL не защищает от рассинхронизации бизнес-правил по обе
стороны, только от прямого доступа к внутренним файлам.

### 30.5 Уточнение к Residual Risk RR-04

`docs/IRP_v1.md` RR-04 указывает контроль «Ревью импортов в CI (flake8
layer rules)» как механизм, удерживающий эти границы. На 2026-07-01
такого механизма не существует: `pyproject.toml` содержит только
`[tool.flake8] max-line-length`/`exclude`, без плагина уровня
import-linter или собственных layer-правил. Границы §22/30 сейчас держатся
исключительно review-дисциплиной (README.md каждого слоя описывает «что
НЕ добавлять сюда»), не автоматической проверкой. Формализация через
`import-linter` или аналог — не входит в scope D02 (Technical Writer/
Principal Architect формат этой задачи не включал добавление нового CI
шага); фиксируется здесь как отдельный, ещё не заведённый пункт для
Wave 4+ или отдельного тикета.
