...
| D02 | DOM08: Anti-Corruption Layers не описаны | docs/BLUEPRINT_ADDENDUM.md | Wave 4 |
...
| **Стратегия** | Обновить ADDENDUM §18.4: переименовать «synthesis_cache_builder.py» → «rebuild_synthesis.py (функция build_cache())». Добавить примечание: «Функционально совмещён с rebuild_synthesis.py — это обоснованное отклонение от §18.4, не технический долг» |
...
| **Трудоёмкость** | 1 час |
| **Стратегия** | Принятое решение (ADR-013): `config/settings.py` остаётся единственным runtime-источником весов для Python. `ontology.json` секция `weight_scores` помечается как `"_note": "display only, не используется в вычислениях — единственный источник: config/settings.py"`. Добавить тест `test_ontology_settings_consistency.py` (уже существует) проверку что значения совпадают |
...
- [ ] Secrets rotation policy описана в SECURITY.md
