...
> ⚠️ **Проверка перед Wave 5 (2026-07-02):** полный кросс-чек всех 26 FAIL
> из оригинального `docs/IRR_REPORT_v1.md` против волн 1-4 и RR-01–RR-08
> нашёл **два FAIL без назначенной волны и без статуса Residual Risk**:
>
> - **MON05** — Dashboards (изначально: «Нет»)
> - **MON07** — Error rate monitoring (изначально: «Только logs в stdout»)
>
> Ни один Wave-таск их не покрывает, ни RR-01–RR-08 их не принимает как
> осознанный риск — это не «забытая, но безопасная» вещь, а пробел в
> самом IRP: 26 исходных FAIL, 19 закрыты волнами, 5 приняты как RR
> (AI08→RR-01, TST13/14/15→RR-02, MON06→RR-03), 2 (MON05, MON07)
> остаются неучтёнными. Критерий §11 «Итого FAIL: ≤ 5 (только из
> RR-01–RR-08)» не будет выполнен, пока это не решено — либо
> реализацией (новый Wave-таск), либо явным добавлением RR-09/RR-10 с
> обоснованием почему допустимо. Решение — не техническое, требует
> Principal QA Architect/Technical PM, не принимается здесь заранее.
