...
// 2026-08-02: ВРЕМЕННАЯ диагностика (убрать после выяснения причины) —
// пользователь тестирует с телефона без доступа к консоли разработчика,
// console.warn не помогает. alert() виден без каких-либо инструментов.
function crosslinkGo(targetId) {
  const el = document.getElementById(targetId);
  if (!el) {
    alert('ДИАГНОСТИКА: элемент "' + targetId + '" не найден в DOM');
    console.warn('crosslinkGo: элемент с id="' + targetId + '" не найден в DOM на момент клика — переход невозможен');
    return;
  }
  alert('ДИАГНОСТИКА: элемент найден, скроллю к "' + targetId + '"');
