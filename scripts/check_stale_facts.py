...
# 2026-09-14: раз число становится "устаревшим" значением (superseded)
# отслеживаемого ключа, чистый текстовый поиск подстроки неизбежно иногда
# совпадёт с НЕ-фактовым числом того же вида в другом месте сайта —
# конкретный пример: strive.btc_holdings=20000 устарело (обновлено до
# 25000, STR-2026-0914-001), а "20 000" голым текстом также встречается
# в высоте блока халвинга 2016 года (#420 000) и в ATH-цене BTC 2017 года
# ($20 000) — оба протокольные/исторические числа, не меняются со временем
# и не имеют отношения к резервам Strive. Ровно случай, который сам скрипт
# просит исключить явно с комментарием (см. main()), а не править вручную
# в index.html — там текст корректен.
KNOWN_NON_FACT_COINCIDENCES = [
    "#420 000",  # высота блока халвинга 2016 — не btc_holdings
    "$20 000",   # ATH цены BTC в 2017 — не btc_holdings
]
...
def strip_safe_spans(html: str) -> str:
    ...
    и известные текстовые совпадения устаревших значений с НЕ-фактовым
    содержимым (см. KNOWN_NON_FACT_COINCIDENCES)."""
    html = FACT_KEY_TAG_RE.sub("", html)
    html = JS_CONST_FALLBACK_RE.sub("", html)
    for known in KNOWN_NON_FACT_COINCIDENCES:
        html = html.replace(known, "")
    return html
