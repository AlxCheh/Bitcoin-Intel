...
def test_open_question_about_staleness_is_documented():
    """
    Правило all_stale → low не является буквальным критерием Р9 (устаревание
    в тексте раздела не названо). Это осознанная интерпретация кода, и она
    вынесена владельцу домена как открытый вопрос.

    Тест держит вопрос видимым: пока он не разрешён, из фикстуры нельзя
    молча убрать его формулировку, а кейсы, стоящие на интерпретации, обязаны
    быть помечены — чтобы никто не принял их за буквальное следование Р9.
    """
    meta = _load()["_meta"]
    assert meta.get("open_question"), "формулировка открытого вопроса удалена из фикстуры"

    interpretive = [c for c in _cases() if c["p9_criterion"] == "interpretation_staleness"]
    assert interpretive, (
        "нет ни одного кейса, помеченного как интерпретация — либо вопрос разрешён "
        "(тогда обнови фикстуру и этот тест), либо пометка потерялась"
    )
    for case in interpretive:
        assert case["inputs"]["all_stale"] is True, (
            f"Кейс {case['id']} помечен интерпретацией устаревания, но all_stale=False"
        )
