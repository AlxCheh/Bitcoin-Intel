...
class TestSharedSubjectConflict:
    ...
    def test_uyazvimost_shared_subject_catches_hashrate_concentration_pair(self):
        """
        2026-07-31, второй проход (реализация плана после обсуждения): найдено
        при разборе оставшихся ошибок extended-датасета —
        INF-2026-0702-001 vs INF-2026-0625-001 делят термин "уязвимость"
        ("увеличивая структурную уязвимость" vs "устраняя... уязвимость"),
        но термина не было в SHARED_SUBJECTS. Не новая попытка — расширение
        покрытия уже проверенного механизма (тот же принцип, что и первый
        случай выше). Проверено: ноль регресса на 73 (65.8%, без изменений),
        +0.9 п.п. на 116 (54.3% -> 55.2%).
        """
        from scripts.contradiction_detector import semantic_inverse_score

        a = (
            "Крупные финансовые группы предпочитают низкокапитальные "
            "комиссионные бизнесы прямому владению майнинг-инфраструктурой "
            "— уход SBI из пул-бизнеса дальше концентрирует хешрейт у "
            "топовых операторов, увеличивая структурную уязвимость сети к "
            "цензуре несмотря на прогресс Stratum V2"
        )
        b = (
            "Децентрализация Bitcoin-майнинга переходит от теории к "
            "практике: контроль над транзакционным слоем начинает "
            "смещаться от пулов к майнерам, устраняя главную регуляторную "
            "уязвимость сети"
        )
        score = semantic_inverse_score(a, b)
        assert score >= 0.5, (
            f"Ожидался score >= 0.5 — общий термин 'уязвимость' с "
            f"противоположными глаголами (увеличивая vs устраняя), "
            f"получено {score}"
        )

    def test_prodazha_frequency_pattern_catches_temporal_reversal(self):
        """
        2026-07-31, третий проход: STR-2026-0706-001 vs STR-2026-0720-001 —
        общий предмет "прода(жа/ёт)" с противоположной ЧАСТОТНОЙ (не
        величинной) характеристикой: "регулярно продаёт" (учащается) vs
        "продажа... становится исключением, не нормой" (становится редкой).
        Отличается от паттерна "уязвимость"/первого случая — там был общий
        предмет + глагол величины (снижает/усиливает), здесь общий предмет +
        частотный дескриптор. Проверено на обоих датасетах через
        scripts/compare_contradiction_precision.py перед принятием: ноль
        регресса на blocking (65.8%), +0.7 п.п. на extended (54.3% -> 55.0%).
        Другие оставшиеся пропуски (MAC-2026-0624-001 vs STR-2026-0623-004 и
        т.д.) рассмотрены и НЕ форсированы — нет столь же чистого общего
        термина, натяжка дала бы менее уверенную пару.
        """
        from scripts.contradiction_detector import semantic_inverse_score

        a = (
            "Дивидендные обязательства Digital Credit перешли из "
            "теоретического риска в измеримую, растущую практику: "
            "крупнейшее корпоративное BTC-казначейство теперь регулярно "
            "продаёт часть резерва для обслуживания собственной долговой "
            "конструкции — модель «накопление через долг» частично "
            "инвертировалась в «частичная ликвидация для обслуживания долга»"
        )
        b = (
            "Strategy институционализировала дилюцию акций как основной "
            "инструмент финансирования дивидендных обязательств Digital "
            "Credit Framework, оставляя BTC-резерв как последний рубеж — "
            "пока ATM-лимит остаётся глубоким, прямая продажа BTC "
            "становится исключением, не нормой"
        )
        score = semantic_inverse_score(a, b)
        assert score >= 0.5, (
            f"Ожидался score >= 0.5 — общий предмет 'прода' с "
            f"противоположной частотой (регулярно vs становится "
            f"исключением/не нормой), получено {score}"
        )

    def test_prodazha_same_direction_no_false_positive(self):
        """
        Защитный тест: 'прода' — короткий, частый корень, риск ложных
        срабатываний выше, чем у более длинных предметов ('уязвимость',
        'структурное давление продаж'). Проверяет, что одинаковое
        направление (оба "регулярно") не даёт конфликта.
        """
        from scripts.contradiction_detector import _shared_subject_conflict_score
        a = "Компания регулярно продаёт часть резерва для дивидендов."
        b = "Другая компания тоже регулярно продаёт BTC для операционных нужд."
